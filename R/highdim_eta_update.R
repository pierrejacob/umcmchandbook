...
#' where v is t_dist_df and m is rate.
#' @export
eta_update <- function(xi, sigma2, beta, eta, t_dist_df, nrepeats=1){
...
#' @export
crn_eta_update <- function(xi_1, Beta_1, eta_1, sigma2_1,
...
maxcoupling_trunc_poly_exp <- function(poly_exponent, rate_1, rate_2,
                                      trunc_upper_1, trunc_upper_2){
  rp <- function(n) r_trunc_poly_exp(poly_exponent, rate_1, trunc_upper_1)
  rq <- function(n) r_trunc_poly_exp(poly_exponent, rate_2, trunc_upper_2)
...
# Maximal-coupling based Eta update
maxcoupling_eta_update <- function(xi_1, Beta_1, eta_1, sigma2_1,
                                         xi_2, Beta_2, eta_2, sigma2_2,
                                         t_dist_df){
  p <- length(eta_1)
  rate_1 <- (Beta_1^2)*(xi_1)/(2*sigma2_1)
  ...
  for (j in 1:p){
    etas_[j,] <-
      maxcoupling_trunc_poly_exp((1+t_dist_df)/2, rate_1[j], rate_2[j],
...
#' @return Returns (eta_1, eta_2) under common random numbers coupling
#' @export
swithtocrn_eta_update <- 
  function(xi_1, Beta_1, eta_1, sigma2_1, xi_2, Beta_2, eta_2, sigma2_2, t_dist_df){
    p <- length(eta_1)
    eta_sample <- cbind(rep(NA,p), rep(NA,p))
    ordered_components <- sample(c(1:p)) # randomize order of updates
    ## max coupling of etas
    for (i in 1:p){
      eta_sample[ordered_components[i],] <- 
        maxcoupling_eta_update(xi_1, Beta_1[ordered_components[i]], eta_1[ordered_components[i]], sigma2_1,
                               xi_2, Beta_2[ordered_components[i]], eta_2[ordered_components[i]], sigma2_2, 
                               t_dist_df)
      if (eta_sample[ordered_components[i],1] != eta_sample[ordered_components[i],2]){
        ## meeting failed
        break
      }
    }
    ## revert to CRN when any meeting has failed
    if (i < p){
      eta_sample[ordered_components[(i+1):p],] <- 
        crn_eta_update(xi_1, Beta_1[ordered_components[(i+1):p]], eta_1[ordered_components[(i+1):p]], sigma2_1,
